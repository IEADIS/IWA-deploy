...
abs.best.approach.rand <- function(data,abs.model.year.reg,abs.model.year.cla,asig,year.test,year.pred){
  abs.model <- NULL
  best.results <- data.frame(Asig = asig, YearPred = year.pred, L.Approach = NA,
                             F1 = NA, Acc = NA, Spec = NA, Mean = NA, stringsAsFactors = FALSE)
  if (!is.null(abs.model.year.reg) & !is.null(abs.model.year.cla)) {
    rand.num <- sample(1:2,1)
    if (rand.num == 1) { # SELECT REGRESSION
      abs.model <- abs.model.year.reg
      best.results$L.Approach <- 'Regression'
      best.results$Mean <- as.numeric(colMeans(abs.model.year.reg$results['Rsquared']))
    }else{ # SELECT CLASSIFICATION
      abs.model <- abs.model.year.cla
      best.results$L.Approach <- 'Classification'
      best.results$Mean <- as.numeric(colMeans(abs.model.year.cla$model$results['Accuracy']))
    }
  ...
  return(list(bestmodel = abs.model, results = best.results, data = data))
}
...
abs.best.approach <- function(data,abs.model.year.reg,abs.model.year.cla,asig,year.test,year.pred){
  ...
  if (nrow(data.filtered) == 0) {
    return(abs.best.approach.rand(data.filtered,abs.model.year.reg,abs.model.year.cla,asig,year.test,year.pred))
  }
  if (!is.null(abs.model.year.reg) & !is.null(abs.model.year.cla)) {
...
abs.models.selection <- function(data, regress.models, classif.models, asignatures, years.pred){
  ...
  for (asig in asignatures) {
    ...
    for (year in years.pred) {
      ...
      if (is.null(abs.model)){
        print(paste('Impossible to deploy a model for, insufficient data:',asig,year))
        next()
      }
