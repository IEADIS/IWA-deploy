...
models.mean.lm <- function(models, test.results, asignature){
  ...
  # METRICS MEAN
  if (is.na(rmse.test) | is.na(r2.test)) {
    rmse.mean <- rmse.cv
    r2.mean <- r2.cv
  }else{
...
deploy.lm <- function(allData,asig,time.start,time.end){
  set.seed(123)
  # ADQUISITION
  ...
  # TRAINING
  if (nrow(data.trans.train) < 10) {return(NULL)}
  ...
  # SAVING DATA
  model.data <- data.frame(Asignature = asig,
...
regress.bestModel.train <- function(allData, asignatures, allowed.years = c()){
  # DATA ADQUIRE
  ...
  for (asignature in asig.sistemas) { # BY ASIGNATURE
    ...
    for (year.pred in years.num) { # BY YEAR TO PREDICT
      if ( ((year.pred - min(years.num)) < 2) | !(year.pred %in% strtoi(allowed.years)) ) { next() }# IF THE MIN DIFF IS LESS THAN 2, CANT BE TRAIN
      ...
      for (year.diff in year.time.lambdas) { # BY TIME WINDOW / YEAR LAMBDA
        asig.model <- deploy.lm(allData,asignature,toString(year.pred-year.diff),toString(year.pred-1))
  ...
  for (asignature in unique(models.comp$Asignature)) {
    ...
    for (year.topred in names(models.data[[asignature]])) {
      ...
      for (year.lambda in names(models.data[[asignature]][[year.topred]])) {
        ficl <<- models.data[[asignature]][[year.topred]]
