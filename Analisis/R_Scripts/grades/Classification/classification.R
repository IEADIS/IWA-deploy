...
getBestDeltaTimeByAsig <- function(allData, init.time, final.time, asig, allowed.years = c(init.time:final.time)){
  
  models <- list(); allmodels <- list();
  list.model <- 1; current.time.final <- final.time;
  while ( current.time.final > init.time ){
    if ( !((current.time.final+1) %in% strtoi(allowed.years)) ) { current.time.final <- current.time.final - 1; next() }
...
classif.bestModel.train <- function(allData, asignatures, allowed.years = c()){
  ...
  for ( asig in asignatures ){
    print(asig)
     model.asig <- getBestDeltaTimeByAsig(allData, init.time, final.time, asig, allowed.years)
